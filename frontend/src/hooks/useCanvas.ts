...
export function useCanvas({ isDrawer }: { isDrawer: boolean }) {
  ...
  const floodFill = useCallback(
    (sx: number, sy: number, fillColor: string): void => {
    ...
    const onStart = (e: MouseEvent | TouchEvent): void => {
      ...
      const pt =
        e instanceof MouseEvent
          ? getPoint(e)
          : getPoint((e as TouchEvent).touches[0]);
  ...
  useEffect(() => {
    ...
    const onDrawData = (data: {
      ...
      if (data.type === "draw_start") {
        ...
        // If we missed draw_start, request full canvas state from server
        if (!cur) {
          getSocket().emit("get_room_state", {}, (res: any) => {
            if (res?.strokes) replayStrokes(res.strokes);
          });
          return;
        }
        const nx = data.x ?? 0,
  ...
  const saveDrawing = useCallback((word?: string): void => {
    ...
    if (word) {
      ...
      ctx.fillText(
        `✏️ "${word}"  —  Skribbl Clone`,
        out.width / 2,
        src.height + labelHeight / 2,
      );
    }
    ...
    link.download = word
      ? `skribbl-${word.replace(/\s+/g, "_")}.png`
      : "skribbl-drawing.png";
