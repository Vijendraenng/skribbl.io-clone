...
export default function GamePage() {
  ...
  const { room, game, playerId, currentWord, currentHint, wordChoices, roundEnd, gameOver, nickname } = useGame();
    ...
    <>
      ...
      <div className="hidden lg:flex h-screen bg-game-bg flex-col">
        ...
        <div className="bg-game-card border-b border-game-border px-4 py-2 flex items-center gap-3 shrink-0">
          ...
          <span className="text-gray-400 text-sm">R{(game.currentRound || 0) + 1}/{game.totalRounds}</span>
          ...
          {game.currentDrawerName && (
            <span className="text-sm text-gray-300 ml-auto">
              {isDrawer ? "✏️ You're drawing!" : `✏️ ${game.currentDrawerName} is drawing…`}
            </span>
          )}
        </div>
