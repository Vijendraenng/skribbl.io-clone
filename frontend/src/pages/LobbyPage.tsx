...
export default function LobbyPage() {
  ...
  const activePlayers = room.players.filter(
    (p) => p.isConnected && p.role !== "spectator",
  );
      ...
      <div className="bg-game-card border border-game-border rounded-2xl p-8 w-full max-w-2xl shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-right text-sm text-gray-400 hidden sm:block">
            <div>
              {activePlayers.length}/{room.settings.maxPlayers} players
            </div>
            {spectators.length > 0 && <div>{spectators.length} spectating</div>}
            <div>
              {room.settings.rounds}r · {room.settings.drawTime}s
            </div>
            {(room.settings as any).difficulty !== "all" && (
              <div className="capitalize text-yellow-400">
                {(room.settings as any).difficulty}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      borderColor: isPlayerHost ? "#f59e0b" : "transparent",
                    }}
                  >
                    {player.avatar}
                  </div>
                  ...
                  <div className="flex gap-1 flex-wrap justify-center">
                    {isPlayerHost && (
                      <span className="text-xs bg-yellow-600/20 text-yellow-400 px-1.5 rounded">
                        👑
                      </span>
                    )}
                    {isMe && (
                      <span className="text-xs bg-blue-600/20 text-blue-400 px-1.5 rounded">
                        You
                      </span>
                    )}
                    {isSpectator && (
                      <span className="text-xs bg-purple-600/20 text-purple-400 px-1.5 rounded">
                        👁 Watch
                      </span>
                    )}
                    {player.isReady && !isSpectator && (
                      <span className="text-xs bg-green-600/20 text-green-400 px-1.5 rounded">
                        ✓ Ready
                      </span>
                    )}
                  </div>
          ...
          {Array.from({
            ...
            .map((_, i) => (
              ...
              >
                <div className="w-12 h-12 rounded-full bg-game-border flex items-center justify-center text-2xl">
                  ?
                </div>
        ...
        <div className="bg-game-bg rounded-xl p-4 grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 text-center text-sm">
          {[
            ...
            {
              label: "Difficulty",
              value: (room.settings as any).difficulty || "all",
            },
            {
              label: "Hints",
              value: room.settings.hintsEnabled ? "On" : "Off",
            },
          ].map((s) => (
            <div key={s.label}>
        ...
        <div className="space-y-3">
          ...
          {!isHost && myPlayer && !myPlayer.isSpectator && (
            <button
              onClick={() => {
          ...
          {isHost && (
            <div>
              ...
              {startError && (
                <p className="text-game-accent text-sm text-center mt-2">
                  {startError}
                </p>
              )}
