...
class SocketHandler {
  ...
  attach(socket) {
    ...
    socket.on("join_spectator", (d, cb) => this._onJoinSpectator(socket, d, cb));
    socket.on("reconnect_room", (d, cb) => this._onReconnectRoom(socket, d, cb));
  ...
  _onCreateRoom(socket, data, callback) {
    try {
      ...
      const room = gameManager.createRoom({ hostId: playerId, settings: settings || {}, io: this.io });
      ...
      callback?.({ success: true, roomCode: room.roomCode, playerId, room: room.toJSON(), passcode: room.passcode || null });
  ...
  _onJoinRoom(socket, data, callback) {
    try {
      ...
      if (!roomCode?.trim() || !nickname?.trim()) return callback?.({ error: "Room code and nickname required" });
      ...
      if (!existingPlayer && room.status === "playing") {
        return callback?.({ error: "Game in progress — you can join as a spectator" });
      }

      const playerId = existingId || uuidv4();
      const player = room.addPlayer({ id: playerId, socketId: socket.id, nickname: nickname.trim(), avatar });

      gameManager.registerSocket(socket.id, room.roomCode);
      gameManager.cancelDeletion(room.roomCode);
      socket.join(room.roomCode);

      room.broadcastExcept(socket.id, "player_joined", { player: player.toJSON(), players: room.getPlayerList() });
  ...
  _onJoinSpectator(socket, data, callback) {
    try {
      ...
      const playerId = uuidv4();
      const player = room.addPlayer({ id: playerId, socketId: socket.id, nickname: nickname.trim(), avatar, role: "spectator" });
  ...
  _onReconnectRoom(socket, data, callback) {
    try {
      ...
      const player = room.addPlayer({ id: playerId, socketId: socket.id, nickname: nickname || "Player", avatar: avatar || "🎨" });
      gameManager.registerSocket(socket.id, room.roomCode);
      gameManager.cancelDeletion(room.roomCode);
      socket.join(room.roomCode);

      room.broadcastExcept(socket.id, "player_reconnected", { player: player.toJSON(), players: room.getPlayerList() });
      callback?.({ success: true, room: room.toJSON(), game: room.game ? room.game.toJSON() : null, strokes: room.game ? room.game.strokes : [] });
  ...
  _onStartGame(socket, data, callback) {
    try {
      ...
      if (!player || player.id !== room.hostId) return callback?.({ error: "Only the host can start" });
  ...
  _onKickPlayer(socket, data, callback) {
    try {
      ...
      room.broadcast("player_kicked", { playerId: targetPlayerId, players: room.getPlayerList() });
      callback?.({ success: true });
    } catch (err) { callback?.({ error: err.message }); }
  }
  ...
  _onPlayerReady(socket, data) {
    ...
    room.broadcast("player_ready_update", { playerId: player.id, isReady: player.isReady, players: room.getPlayerList() });
  }

  _onGetRoomState(socket, data, callback) {
    const room = data?.roomCode ? gameManager.getRoom(data.roomCode) : gameManager.getRoomBySocket(socket.id);
    if (!room) return callback?.({ error: "Room not found" });
    callback?.({ room: room.toJSON(), game: room.game ? room.game.toJSON() : null, strokes: room.game ? room.game.strokes : [] });
  }
  ...
  _onCanvasFill(socket, data) {
    ...
    const drawer = room.game.currentDrawer;
    if (!drawer || drawer.socketId !== socket.id) return;
    const stroke = { type: "canvas_fill", x: data.x, y: data.y, color: data.color };
    room.game.addStroke(stroke);
  ...
  _onDrawShape(socket, data) {
    ...
    const stroke = {
      type: "draw_shape",
      shapeType: data.shapeType, // "line" | "rect" | "circle"
    ...
    room.game.addStroke(stroke);
    // Broadcast to everyone including drawer (for consistency)
    room.broadcast("draw_shape", stroke);
  }
  ...
  _onGuess(socket, data) {
    ...
    if (result.correct) {
      room.sendTo(socket.id, "guess_result", { correct: true, points: result.points, word: room.game.currentWord });
      room.broadcast("player_guessed", { playerId: player.id, playerName: player.nickname, points: result.points, players: room.getPlayerList() });
    } else {
      if (result.isClose) room.sendTo(socket.id, "close_guess", { text });
      const drawer = room.game?.currentDrawer;
      if (drawer) {
        room.broadcastExcept(drawer.socketId, "chat_message", { playerId: player.id, playerName: player.nickname, text, type: "guess" });
      } else {
        room.broadcast("chat_message", { playerId: player.id, playerName: player.nickname, text, type: "guess" });
      }
    }
  }
  ...
  _onChat(socket, data) {
    ...
    room.broadcast("chat_message", { playerId: player.id, playerName: player.nickname, text, type: "chat" });
  }
  ...
  _onPlayAgain(socket, data, callback) {
    try {
      ...
      if (!room) return callback?.({ error: "Not in a room" });
      const player = room.getPlayerBySocket(socket.id);
      if (!player || player.id !== room.hostId) return callback?.({ error: "Only host can restart" });

      room.status = "waiting";
      room.game = null;
      for (const p of room.players.values()) {
        p.score = 0; p.roundScore = 0; p.hasGuessedCorrectly = false; p.isReady = false;
      }
      ...
      if (room.game && room.game.phase === "drawing") {
        ...
        if (connectedInGame.length < 2) {
          ...
          if (!drawer || !drawer.isConnected) {
            setTimeout(() => { if (room.game?.phase === "drawing") room.endRound(); }, 1500);
          }
        }
      }
  ...
  _onDisconnect(socket) {
    ...
    if (room.game && room.game.phase === "drawing") {
      ...
      const drawer = room.game.currentDrawer;
      if (!drawer || !drawer.isConnected) {
        if (room._chooseTimeout) { clearTimeout(room._chooseTimeout); room._chooseTimeout = null; }
